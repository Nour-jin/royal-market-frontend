...
const Item = () => {
  ...
  useEffect(() => {
    ...
    if (product.oldPice) {
      const fullPrice = product.oldPice.toString().split(".");
      const price = fullPrice[0];
      const cent = fullPrice[1];
      product.oldcent = cent;
      product.oldPice = price;
    }
  }, [product]);
    ...
    <>
      ...
      <div className="item">
        ...
        <div
          ...
          style={{
            backgroundColor: product.img
              ? product.img[count].color.Muted
              : "black",
          }}
        />
        {edit ? (
          ...
          <div className="item-info">
            ...
            <div className="item-body">
              <div className="item-price">
                <h5>
                  <div
                    className={product.oldPrice ? "oldPrice" : "price-normal"}
                  >
                      {product.price}<span className={product.oldPrice ? "" : "cent"}> {product.cent ? "," :""}{product.cent}<span> €</span></span>
                      
                  </div>
                  
                   {product.oldPrice ? <div className="newPrice">
                       {product.oldPrice}
                  {product.oldcent ? <span className="cent">, {product.oldcent}</span> :""}
                  <span> €</span>
                    </div> :""}
